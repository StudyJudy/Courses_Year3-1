...
DWORD SR_receive_FSM(char* file, char* filename, int N)
{
	...
	while (1)
	{
		...
		// 在窗口范围内，
		// 发送方一次传输N个分组过来，当接收方正确接受到其中任意一个分组，不管是否按序，接收方都会返回一个ACK。
		// 收到一个帧就将该帧缓存到窗口中并回复一个ack
		// 如果接收到的包没有丢失现象（是连续的），交付给上一层
		// 接收到顺序帧后将数据提交给上层并接收窗口后移
		else if (rec->seq_num >= base && rec->seq_num < base + N && rec->seq_num == base && vertifyChecksum(rec, &ph))
		{
			...
			// 发送确认报文
			sendto(sockServer, sendBuffer, sizeof(msg), 0, (sockaddr*)&addr_client, len);
			cout << "Send\tACK: \t" << isAck(&ackpkt) << "\tack: \t " << base << "\tchecksum: \t" << ackpkt.checksum << "\texpectedseqnum: \t" << base + 1 << "\tlength: \t" << ackpkt.length << " (按序接收，已交付)" << endl;

			// 将接收到的数据包的数据复制到文件缓冲区,将数据交付给上一层
			...
			while (ReceiverBufferFlags[(++base) % N])
			{
				...
				cout << "Deliever\t" << "\tack: \t " << base << "\tchecksum: \t" << bufferedPacket.checksum << "\texpectedseqnum: \t" << base + 1 << "\tlength: \t" << bufferedPacket.length << " (失序缓存数据包已交付)" << endl;
		...
		// 失序先缓存
		// 如果接收到的包失序不连续，缓存窗口范围内后续收到的包，并且返回给发送方一个该帧的确认帧（收谁确认谁）
		// 若接收到的帧不是连续的顺序帧时接收窗口不移动
		// 在接收方缓存的失序分组将被缓存知道所有的丢失分组（即序号更小的分组）都被接受为止，然后才将这一批分组按序交付给上一层。
		// 然后向前移动滑动窗口	
		else if (rec->seq_num > base && rec->seq_num < base + N && vertifyChecksum(rec, &ph))
		{
			ReceiverBufferFlags[rec->seq_num % N] = true;//标记已缓存
			buffer.push_back(*rec);// 缓存
			...
			// 发送确认报文
			sendto(sockServer, sendBuffer, sizeof(msg), 0, (sockaddr*)&addr_client, len);		
			cout << "Send\tACK: \t" << isAck(&ackpkt) << "\tack: \t " << rec->seq_num << "\tchecksum: \t" << ackpkt.checksum << "\texpectedseqnum: \t" << base << "\tlength: \t" << ackpkt.length << "（确认接收但失序先缓存）" << endl;
		}

		// 如果收到了窗口外小于窗口下界的帧，就返回一个ACK
		// 其他接收到窗口外的包情况则应该忽略丢弃这个帧
		else if (rec->seq_num >= base - N && rec->seq_num <= base - 1)
		{
			// 接收到窗口外的数据包，返回确认,构造确认包
			msg ackpkt = make_pkt(rec->seq_num);
			// 将确认报文复制到发送缓冲区
			memcpy(sendBuffer, &ackpkt, sizeof(msg));
			// 发送确认报文
			sendto(sockServer, sendBuffer, sizeof(msg), 0, (sockaddr*)&addr_client, len);
			cout << "Send\tACK: \t" << isAck(&ackpkt) << "\tack: \t " << base - 1 << "\tchecksum: \t" << ackpkt.checksum << "\texpectedseqnum: \t" << base << "\tlength: \t" << ackpkt.length << "（已过时）" << endl;
			
		}
		// 其他窗口外的情况则忽略并丢弃这个数据包
		else
		{
			cout << "WRONG！！！Receiver端预期的序号应该在 [ " << base << " ," << nextseqnum << " ] " << "之间" << endl;
		}
