...
int base = 1;// 发送窗口的基序号，标识窗口的起始位置，下一个未确认的分组序号
...
bool* sent;        // 用于标记已发送但未确认的数据包
bool* timeout;     // 计时器状态，用于标记已发送但未确认的数据包是否超时

clock_t* timers;   //用于记录每个数据包的计时器，全局变量自动赋0，记录每个未确认数据包的计时器启动时间

// 在 SR 发送状态机外部定义 acked 数组
bool* acked;
...
DWORD WINAPI RecHandle(LPVOID param)
{
	...
	while (rec_stage)
	{
		...
		// 接收到ack后，将被确认的帧标记为已接收

		// 如果（顺序帧）这个帧的序号是窗口的下界，窗口向前移动到具有最小序号的未确认帧处
		if (isACK(rec) && vertifyCheckSum(rec, &ph) && (rec->ack_num == base) && (rec->ack_num) < base + N)
		{
			...
			{
				printf("Receive\tack:\t%d\tchecksum:\t%d\tbase: \t%d\tnextseqnum: \t%d\tlength: \t%d\tACK: \t%d\t窗口边界: \t[%d, %d]\n", rec->ack_num, rec->checksum, base, lastseqnum, rec->length, isACK(rec), base, base + N - 1);
			}
			else
			{
...
void SR_send_FSM(unsigned long length_file, char* file, char* filename, int N)
{
	...
	acked = new bool[packet_num + 1];// 标记是否确认
	memset(acked, 0, packet_num + 1);

	timeout = new bool[packet_num + 1];// 计时器状态，用于标记已发送但未确认的数据包是否超时
	memset(timeout, 0, packet_num + 1);

	timers = new clock_t[packet_num + 1];// 计时器，记录每个未确认数据包的计时器启动时间
...
DWORD WINAPI RecHandle(LPVOID param)
{
	...
	while (rec_stage)
	{
		...
		// 检查接收到的消息是否是确认消息，且未损坏，且确认号=base窗口下界
		// 接收到ack后，将被确认的帧标记为已接收
		// 如果（顺序帧）这个帧的序号是窗口的下界，窗口向前移动到具有最小序号的未确认帧处
		...
		// 检查接收到的消息是否是确认消息，且未损坏，且确认号在窗口范围内，但不连续是乱序收到的
		else if (isACK(rec) && vertifyCheckSum(rec, &ph) && (rec->ack_num > base) && (rec->ack_num < base + N))
		{
			// 标记这个包已收到确认
			markAsACKed(rec->ack_num);
